...
from app.schemas.sales import (
    SaleParseRequest, SaleParseResponse, SaleConfirmPayload, SaleResponse
)
...
@router.post("/confirm", response_model=SaleResponse, status_code=status.HTTP_201_CREATED)
def confirm_sale(req: SaleConfirmPayload, db: Session = Depends(get_db)):
    """
    ...
    try:
        ...
        margin_pct = round((profit / total_amount * 100), 2) if total_amount > 0 else 0.0
        ...
        if isinstance(e, HTTPException):
            raise e
        raise HTTPException(status_code=400, detail=f"Sale transaction failed: {str(e)}")

@router.get("", response_model=List[SaleResponse])
def list_sales_history(shop_id: str = "shop_001", db: Session = Depends(get_db)):
